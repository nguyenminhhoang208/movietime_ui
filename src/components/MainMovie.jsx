...
const MainMovie = ({ title, movie, hoverNextSlide, isSlideChild = false}) => {
	const starArray = [];
	...
	return (
		<div className='w-full mb-10 mt-6 animate-showMainMovie'>
			{title ? (
			...
			<div className='w-[100%]  rounded-lg overflow-hidden relative shadow-md shadow-black group/mainMovie'>
				...
				<div
					onMouseMove={() => {
						if(isSlideChild) hoverNextSlide.setNextSlide(false);
					}}
					onMouseOut={() => {
						if(isSlideChild) hoverNextSlide.setNextSlide(true);
					}}
            ...
            transition ease-in-out duration-[1.25s] '
				...
				<div
					...
					className='lg:scale-[1] hidden md:inline-block md:scale-[0.65] h-[422px]  md:absolute bottom-[-65px] left-[-40px] md:group-hover/movie:left-[-108px]  lg:bottom-3 lg:left-3 p-1 rounded-lg overflow-hidden bg-[#30303048] shadow-lg shadow-black group/movie'
				>
